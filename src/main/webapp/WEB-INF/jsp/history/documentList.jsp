...
<div id="contents">
    ...
    <div class="container">
        ...
        <input type="hidden" id="headYear" name="headYear" value="<c:out value="${paramMap.headYear }"/>"/>
        ...
        <section id="fieldset">
            ...
            <div class="mMenu1 type2 nonavi">
                ...
                <!-- //depth1 -->
                <!-- depth2 -->
                <c:if test="${fn:length(headYearList) > 0 }">
                    <div class="owl-carousel dep2" id="archives-tab-dep2-1">
                        <c:forEach var="item" items="${headYearList }">
                            <a href="javascript:fcnHeadYearSearch('<c:out value="${item.year }"/>');" <c:if test="${paramMap.headYear eq item.year }">class="selected"</c:if>><c:out value="${item.year }"/></a>
                        </c:forEach>
                    </div>
                </c:if>
                <!-- //depth2 -->
            ...
            <div class="mSearch">
                ...
                <div class="dbinfo-radio selector-wrap horizontal">
                    ...
                    <div class="asset type2">
                        <label><input type="radio" name="searchType" id="searchType_TARGET" value="TARGET" <c:if test="${paramMap.searchType == 'TARGET'}">checked="checked"</c:if>> <span class="label">생산기관</span></label>
                    </div>
                </div>
                <a href="javascript:fcnDocumentIntro();" class="iFeel type2"><span>공문서 소개</span></a>
            </div>
            <!-- //search -->

            <!-- sort -->
            <div class="mSort1">
                <h4>공문서 (<span class="em weighty"><c:out value="${pageListVo.totalCount }"/></span>)</h4>
                <div class="rt">
                 ...
                 <c:forEach var="item" items="${pageListVo.resultList }" varStatus="status">
                    <li>
	                    ...
	                    <span class="text">
	                        ...
	                        <span class="des"><c:out value="${item.summaryDesc }" escapeXml="false"/></span>
	                        <span class="lst">
		                        <fmt:parseDate value="${item.birthDate}" var="fmtBirthDate" pattern="yyyyMMdd"/>
	                            <span class="ls char4"><em>생산일</em> <fmt:formatDate value="${fmtBirthDate}" pattern="yyyy-MM-dd"/></span>
	                            <span class="ls char4"><em>생산기관</em> <c:out value="${item.targetName }"/></span>
	                        </span>
	                        <span class="link">
...
$(document).ready(function(){
    ...
    $("#_pageSize, #searchOrderType").bind("change", function() {
    	$("#_page").val("1");
        formSubmit("form1", "<c:url value="/history/document"/>");
    });
});
...
function fcnHeadYearSearch(pHeadYear) {
    $("#headYear").val(pHeadYear);
    $("#year").val("");
...
function fcnDocumentIntro() {
    formSubmit("form1", "<c:url value="/history/document/intro"/>");
}
</script>