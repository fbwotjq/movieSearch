	                ...
	                <c:forEach var="item" items="${pageListVo.resultList }" varStatus="status">
	                    <li>
	                        <a href="javascript:fcnDetail('<c:out value="${item.seq }"/>');">
	                            <span class="mImg1"><span style="background-image: url('${KF_FILM_UPLOAD_PATH}<c:out value="${item.fileName }"/>')"></span></span>
	                            <span class="text">
	                                <span class="tit"><c:out value="${item.title }"/></span>
	                                <span class="des"><c:out value="${item.getShortNonTagContent }" escapeXml="false"/></span>
	                                <span class="nam"><span>by.</span>${item.writer }</span>
	                                <fmt:parseDate value="${item.regdt}" var="fmtRegitDate" pattern="yyyyMMdd"/>
