    ...
    <div class="container">
        <div class="top-area type2">
	        ...
	        <div class="submenu-depth3">
	            <h3 class="submenu-depth3-title2"><span><em>영화글</em> 필자목록</span></h3>
	        </div>
        </div>
                            ...
                            <dd>
                                <div class="list">
	                                ...
	                                <c:forEach var="subItem" items="${item.writerDivList }">
	                                    <c:choose>
	                                        <c:when test="${subItem.movieStoryCount <= 0  }">
	                                            <li><a href="#none;"><c:out value="${subItem.writerName }"/><em>(<c:out value="${subItem.movieStoryCount }"/>)</em></a></li>
	                                        </c:when>
	                                        <c:otherwise>
	                                            <li><a href="<c:url value="/story/writer/content/list?writer="/><c:out value="${subItem.writerName }"/>"><c:out value="${subItem.writerName }"/><em>(<c:out value="${subItem.movieStoryCount }"/>)</em></a></li>
	                                        </c:otherwise>
	                                    </c:choose>
	                                </c:forEach>
	                                </ul>
