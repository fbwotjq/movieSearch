...
</c:choose> (<span class="em weighty">${pageListVo.totalCount }</span>)</h4></div>
...
<!-- list -->
<c:if test="${pageListVo.totalCount > 0}">
<div class="result-block-detail mv-review">
	<ul>
			...
			<div class="details">
				<div class="movie-tt">
					<a href="#" class="ftc-blue">
						...
						<div class="vod forPc"><span onclick="javascript:alert('VOD상세로 이동')">VOD</span></div> <!-- forPc -->
						</c:if>
					</a>
				</div>
				<div class="movie-tt-eng"><c:out value='${item.titleEng}'/></div>
				<div class="national">
					<a href="javascript:alert('분류검색으로 이동');" class="ftc-blue"><span><c:out value='${item.typeClss}'/></span></a>,  <a href="#2" class="ftc-blue"><span><c:out value='${item.nationClss}'/></span></a>,
				</div>
				<div class="year">
				...
				<div class="director">
					...
					<a href="javascript:alert('인물상세로 이동');" class="ftc-blue"><c:out value='${item2.personNm}'/></a><c:if test="${!status2.last}">,</c:if>
					</c:forEach>
					</div>
				</div>
				<div class="actor">
					...
					<a href="javascript:alert('인물상세로 이동')" class="ftc-blue"><c:out value='${item3.personNm}'/></a><c:if test="${!status3.last}">,</c:if>
					</c:forEach>
					</div>
				</div>
				<c:if test="${item.cnt > 0}">
				<div class="vod forMobile"><span onclick="javascript:alert('VOD상세로 이동')">VOD</span></div> <!-- forMobile -->
				</c:if>
				<div class="movie_regibtn">
					<a href="#reg" onclick="fncProc('${item.title}','${item.movieId}', '${item.movieSeq}', '${item.nationalClss}');" class="btn medium weighty">등록</a>
				</div>
			</div>
			<div class="poster">
				<a href="javascript:alert('영화상세 페이지로 이동');" class="ftc-blue">
				<c:choose>
