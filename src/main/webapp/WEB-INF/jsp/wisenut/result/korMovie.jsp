...
<div class="result-block pt1">
    <div class="result-block-tt">
        <h4>한국영화 (<span class="em weighty">${resultData.kmKORMOVIECount}</span>)</h4>
    </div>
<c:choose>
    <c:when test="${(collection eq 'ALL' && resultData.kmKORMOVIECount > 0) || collection eq 'kmKORMOVIE'}">
    <!-- list -->
    <div class="result-block-detail">
        <ul>
            <c:forEach var="kmKORMOVIE" items="${resultData.kmKORMOVIEList}" varStatus="status">
                <c:set var="DIRECTOR_LINK" value="${fn:split(kmKORMOVIE.DIRECTOR_LINK,'#@#')}" />
                <c:set var="ACTOR_LINK" value="${fn:split(kmKORMOVIE.ACTOR_LINK,'#@#')}" />
            <li class="detail-box">
<%--
- 런타임 없음.
...
--%>
    ...
    <c:if test="${'kmKORMOVIE' ne collection}">
    <div class="mButton1 mt15">
        <span class="gRight"><a href="javascript:doCollectionSearch('kmKORMOVIE', 'movieTab');" class="iMore1">더보기</a></span>
    </div>
    </c:if>
</div>
<!-- //한국영화 -->