...
<c:if test="${result.result == 'N'}">
<script>alert("${result.msg}"); window.close();</script>
</c:if>
...
<form id="SendPayForm_id" method="POST" >
		...
		<input type="hidden" name="goodname" value="${result.strType}" />
		<input type="hidden" name="oid" value="${oid}" />
		<input type="hidden" name="price" value="${servicePrice}" />
		<input type="hidden" name="currency" value="WON" />
		<input type="hidden" name="buyername"value="${result.companyname}" />
		...
		<input type="hidden" name="nation" value="K"/>
		<input type="hidden" name="p_dataid" value="999999"/>
		<input type="hidden" name="mul_id" value="999"/>
		<input type="hidden" name="file_id" value="999999" />
		<input type="hidden" name="sMovieTitleStringKorean" id="sMovieTitleStringKorean" value="${result.strType}"/>
	...
	<div class="mPopup1 winPop" id="jsPayVod">
		<div class="popupBg">
			...
			<div class="cont">
				<dl class="lst1">
				<dt>상풍명</dt>
				<dd>${result.strType }</dd>
				<dt>관람료</dt>
				<dd><fmt:formatNumber value="${servicePrice}" type="number"/> 원</dd>
				<dt class="to">결제금액</dt>
				<dd class="to"><fmt:formatNumber value="${servicePrice}" type="number"/> 원</dd>
				</dl>
				...
				<dd>
					<div class=" select-box">
						<select name="gopaymethod" id="select" class="select-tric">
							<option value="onlyhpp">휴대전화</option>
				...
				<dd>
					<div class="gIt"><input type="text" id="buyertel" name="buyertel" title="연락처" placeholder="직접입력" value=""></div>
				</dd>
				<dt><label id="buyeremail">이메일</label></dt>
				<dd>
					<div class="gIt"><input type="text" id="buyeremail" name="buyeremail" title="연락처" placeholder="직접입력" value=""></div>
				</dd>
				</dl>
				<ul class="info">
