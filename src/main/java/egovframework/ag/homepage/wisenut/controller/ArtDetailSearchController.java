...
@Controller
public class ArtDetailSearchController {
	// 실시간 검색어 화면 출력 여부 체크
    ...
    /** 작품 상세 페이지 이동 */
    @RequestMapping(value = "/moveArtSearch")
    public String artMoveSearch(HttpServletRequest request, Model model) {
        return "/wisenut/detail/artDetailSearch";
    }

    @RequestMapping(value = "/artSearch")
	public String artDetailsearch(HttpServletRequest request, Model model) {
...
//                exquery = "<WRITER:" + writer + ">";
        ...
        if(collection.equals("ALL")) { //통합검색인 경우
            ...
            // 국내영화, 국외영화
            collections = new String[] {"kmKORMOVIE","kmFORMOVIE"};
        } else {                        //개별검색인 경우
