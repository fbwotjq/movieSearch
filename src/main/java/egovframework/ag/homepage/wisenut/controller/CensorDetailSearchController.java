...
@Controller
public class CensorDetailSearchController {
	// 실시간 검색어 화면 출력 여부 체크
    ...
    /** 심의서류 상세 페이지 이동 */
    @RequestMapping(value = "/moveCensorSearch")
    public String artMoveSearch(HttpServletRequest request, Model model) {
        return "/wisenut/detail/censorDetailSearch";
    }

	@RequestMapping(value = "/censorSearch")
...
//                exquery = "<WRITER:" + writer + ">";
        ...
        if(collection.equals("ALL")) { //통합검색인 경우
            collections = COLLECTIONS;      // 기사평론만 TODO
        } else {                        //개별검색인 경우
